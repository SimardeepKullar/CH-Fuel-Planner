...
**Measured, 2026-09-14 (T-10 step 5).** Live calls against the free tier's `x-ratelimit-limit` header confirm v3.4's figures exactly: **200/day for directions, 50/day for matrix, 100/day for geocoding** — not the 2,000/day and 500/day the documentation quotes. The documented quota is not the operative one; `backend/src/routing/ors.ts`'s `DEFAULT_BUDGET_CEILINGS` sets the budget guard's ceiling from this measurement (200 directions, 50 matrix), not from the docs. `x-ratelimit-reset` moved with each request (a rolling window from the moment of the call, not a fixed UTC clock boundary) — one more reason §8.3's rule stands: store the number and when it was observed, and state no period.
